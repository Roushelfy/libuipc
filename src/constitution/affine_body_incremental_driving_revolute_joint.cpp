...
static constexpr U64 RevoluteJointUID = 18;  // UID of AffineBodyRevoluteJoint
...
REGISTER_CONSTITUTION_UIDS()
{
    ...
    uids.push_back(UIDInfo{.uid  = ConstitutionUID,
                           .name = "AffineBodyIncrementalDrivingRevoluteJoint",
                           .type = string{builtin::Constraint}});
    return uids;
}

AffineBodyIncrementalDrivingRevoluteJoint::AffineBodyIncrementalDrivingRevoluteJoint(const Json& config)
{
    m_config = config;
}

AffineBodyIncrementalDrivingRevoluteJoint::~AffineBodyIncrementalDrivingRevoluteJoint() = default;

void AffineBodyIncrementalDrivingRevoluteJoint::apply_to(geometry::SimplicialComplex& sc)
{
    UIPC_ASSERT_THROW(sc.dim() == 1,
                "AffineBodyIncrementalDrivingRevoluteJoint can only be applied to 1D simplicial complex (linemesh), "
                "but got {}D",
                sc.dim());
    ...
    UIPC_ASSERT_THROW(uid && uid->view()[0] == RevoluteJointUID,
                "Simplicial complex does not have constitution uid {}. "
                "Please apply an AffineBodyRevoluteJoint before applying AffineBodyIncrementalDrivingRevoluteJoint",
                RevoluteJointUID);
...
Json AffineBodyIncrementalDrivingRevoluteJoint::default_config()
{
    return Json::object();
}

U64 AffineBodyIncrementalDrivingRevoluteJoint::get_uid() const noexcept
{
