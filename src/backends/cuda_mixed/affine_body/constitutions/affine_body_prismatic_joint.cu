                ...
                [body_ids = body_ids.cviewer().name("body_ids"),
                 ...
                 rest_ts  = rest_ts.cviewer().name("rest_ts"),
                    ...
                    Es = info.energies().viewer().name("Es")] __device__(int I)
                   {
                       ...
                       // Create Frame F01
                       Eigen::Matrix<Alu, 9, 1> F01;
                       PJ::F01<Alu>(F01,
                                    rest_c.segment<3>(0).template cast<Alu>(),  // ci_bar
                                    rest_t.segment<3>(0).template cast<Alu>(),  // ti_bar
                                    qi,                                          // qi
                                    rest_c.segment<3>(3).template cast<Alu>(),  // cj_bar
                                    rest_t.segment<3>(3).template cast<Alu>(),  // tj_bar
                                    qj);                                         // qj

                       Alu E01;
                       PJ::E01<Alu>(E01, kappa, F01);

                       Alu E23;
                       PJ::E23<Alu>(E23,
                                    kappa,
                                    rest_n.segment<3>(0).template cast<Alu>(),  // ni_bar
                                    rest_b.segment<3>(0).template cast<Alu>(),  // bi_bar
                                    qi,                                          // qi
                                    rest_n.segment<3>(3).template cast<Alu>(),  // nj_bar
        ...
        ParallelFor()
            ...
            .apply(body_ids.size(),
                    ...
                    gradient_only] __device__(int I) mutable
                   {
                       ...
                       Eigen::Matrix<Alu, 9, 1> G01;
                       PJ::dE01dF01<Alu>(G01, kappa, F01);
                       Eigen::Matrix<Alu, 24, 1> J01T_G01;
                       PJ::J01T_G01<Alu>(J01T_G01,
                                         G01,                                         // G01
                                         rest_c.segment<3>(0).template cast<Alu>(),  // ci_bar
                                         rest_t.segment<3>(0).template cast<Alu>(),  // ti_bar
                                         rest_c.segment<3>(3).template cast<Alu>(),  // cj_bar
                       ...
                       Eigen::Matrix<Alu, 24, 1> dE23dQ;
                       PJ::dE23dQ<Alu>(dE23dQ,
                                       kappa,
                                       rest_n.segment<3>(0).template cast<Alu>(),  // ni_bar
                                       rest_b.segment<3>(0).template cast<Alu>(),  // bi_bar
                                       qi,                                          // qi
                       ...
                       if(!gradient_only)
                       {
                           ...
                           Eigen::Matrix<Alu, 9, 9> H01;
                           PJ::ddE01ddF01<Alu>(H01, kappa, F01);

                           // Ensure H01 is SPD
                           make_spd(H01);

                           Eigen::Matrix<Alu, 24, 24> J01T_H01_J01;
                           PJ::J01T_H01_J01<Alu>(J01T_H01_J01,
                                                 H01,
                                                 rest_c.segment<3>(0).template cast<Alu>(),  // ci_bar
                                                 rest_t.segment<3>(0).template cast<Alu>(),  // ti_bar
                                                 rest_c.segment<3>(3).template cast<Alu>(),  // cj_bar
                           ...
                           Eigen::Matrix<Alu, 24, 24> ddE23ddQ;
                           PJ::ddE23ddQ<Alu>(ddE23ddQ,
                                             kappa,
                                             rest_n.segment<3>(0).template cast<Alu>(),  // ni_bar
                                             rest_b.segment<3>(0).template cast<Alu>(),  // bi_bar
                                             qi,                                          // qi
