...
{
template <typename T>
inline UIPC_GENERIC Eigen::Matrix<T, 12, 12>
compute_constraint_mass(const ABDJacobiDyadicMass& mass,
                    ...
                    is_fixed = info.is_fixed().viewer().name("is_fixed")] __device__(int I)
                   {
                       ...
                       else
                       {
                           ...
                           Eigen::Matrix<Alu, 12, 1> dq_alu =
                               (q - q_aim).template cast<Alu>();
                           Vector2  s  = strength_ratios(I);

                           Eigen::Matrix<Alu, 12, 12> M_alu =
                               compute_constraint_mass<Alu>(body_masses(i),
                                                            safe_cast<Alu>(s(0)),
                                                            safe_cast<Alu>(s(1)));
            ...
            .apply(constrained_bodies.size(),
                    ...
                    gradient_only = info.gradient_only()] __device__(int I) mutable
                   {
                       ...
                       else
                       {
                           ...
                           M_alu = compute_constraint_mass<Alu>(body_masses(i),
                                                                safe_cast<Alu>(s(0)),
                                                                safe_cast<Alu>(s(1)));
