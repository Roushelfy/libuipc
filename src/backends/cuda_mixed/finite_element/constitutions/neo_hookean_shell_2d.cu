        ...
        ParallelFor()
            ...
            .apply(info.indices().size(),
                    ...
                    dt       = info.dt()] __device__(int I)
                   {
                       ...
                       for(int i = 0; i < 3; ++i)
                           X.segment<3>(3 * i) = xs(idx(i)).template cast<Alu>();

                       const Alu lambda = safe_cast<Alu>(lambdas(I));
                       const Alu mu     = safe_cast<Alu>(mus(I));

                       const Float thickness_f = triangle_thickness(thicknesses(idx(0)),
                                                                    thicknesses(idx(1)),
                                                                    thicknesses(idx(2)));
                       const Alu rest_area = safe_cast<Alu>(rest_areas(I));
                       const Alu thickness = safe_cast<Alu>(thickness_f);
                       Eigen::Matrix<Alu, 2, 2> IB_alu = IB.template cast<Alu>();
                       Alu E               = Alu{0};
                       NH::E(E, lambda, mu, X, IB_alu);
        ...
        ParallelFor()
            ...
            .apply(info.indices().size(),
                    ...
                    gradient_only = info.gradient_only()] __device__(int I) mutable
                   {
                       ...
                       Eigen::Matrix<Alu, 2, 2> IB_alu = IBs(I).template cast<Alu>();
                       ...
                       NH::ddEddX(H_alu, lambda, mu, X, IB_alu);
                       make_spd(H_alu);
                       H_alu *= Vdt2;
