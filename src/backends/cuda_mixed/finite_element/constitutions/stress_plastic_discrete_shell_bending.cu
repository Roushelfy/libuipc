        ...
        ParallelFor()
            ...
            .apply(info.energies().size(),
                    ...
                    dt           = info.dt()] __device__(int I)
                   {
                       ...
                       Alu      L0           = safe_cast<Alu>(L0s(I));
                       Alu      h_bar        = safe_cast<Alu>(h_bars(I));
                       Alu      theta_bar    = safe_cast<Alu>(theta_bars(I));
                       Alu      yield_stress = safe_cast<Alu>(yield_stresses(I));
                       Alu      V_bar        = safe_cast<Alu>(V_bars(I));

                       Eigen::Matrix<Alu, 3, 1> x0 = xs(stencil[0]).template cast<Alu>();
                       Eigen::Matrix<Alu, 3, 1> x1 = xs(stencil[1]).template cast<Alu>();
                       Eigen::Matrix<Alu, 3, 1> x2 = xs(stencil[2]).template cast<Alu>();
                       Eigen::Matrix<Alu, 3, 1> x3 = xs(stencil[3]).template cast<Alu>();

                       Alu E = SPDSB::E(x0, x1, x2, x3, L0, h_bar, theta_bar, kappa, yield_stress);
        ...
        ParallelFor()
            ...
            .apply(stencils.size(),
                    ...
                    dt           = info.dt()] __device__(int I) mutable
                   {
                       ...
                       Alu Vdt2 = V_bar * safe_cast<Alu>(dt * dt);

                       Eigen::Matrix<Alu, 12, 1>  G12;
                       Eigen::Matrix<Alu, 12, 12> H12x12;
        ...
        ParallelFor()
            ...
            .apply(spdsb->stencils.size(),
                    ...
                    xs = fem->xs().cviewer().name("xs")] __device__(int I) mutable
                   {
                       ...
                       Alu theta = static_cast<Alu>(0.0);
                       if(!SPDSB::safe_dihedral_angle(x0, x1, x2, x3, theta))
                           return;

                       Alu theta_bar         = safe_cast<Alu>(theta_bars(I));
                       Alu yield_stress      = safe_cast<Alu>(yield_stresses(I));
                       Alu hardening         = safe_cast<Alu>(hardening_moduli(I));
                       Alu bending_stiffness = safe_cast<Alu>(bending_stiffnesses(I));
                       Alu L0                = safe_cast<Alu>(rest_lengths(I));
                       Alu h_bar             = safe_cast<Alu>(h_bars(I));

                       if(SPDSB::update_plastic_state<Alu>(theta,
                                                           theta_bar,
                                                           yield_stress,
                                                           hardening,
                                                           bending_stiffness,
                                                           ...
                                                           h_bar))
                       {
                           theta_bars(I)     = safe_cast<Float>(theta_bar);
                           yield_stresses(I) = safe_cast<Float>(yield_stress);
                       }
                   });
    }
