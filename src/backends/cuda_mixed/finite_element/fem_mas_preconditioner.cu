...
void assemble_diag_inv_for_unpartitioned(
    muda::DeviceBuffer<DiagInvMat3x3>& diag_inv,
    const muda::DeviceBuffer<int>&    unpart_flags,
    GlobalLinearSystem::CBCOOMatrixView A,
    ...
    SizeT                             num_verts)
{
    ...
    diag_inv.fill(DiagInvMat3x3::Identity());
    ...
    ParallelFor()
        ...
        .apply(A.triplet_count(),
                ...
                fem_count  = fem_block_count] __device__(int I) mutable
               {
                   ...
                   if(i == j && unpart(i) == 1)
                   {
                       auto H = H3x3.template cast<PcgAuxScalar>().eval();
...
void apply_diag_inv_for_unpartitioned(
    const muda::DeviceBuffer<DiagInvMat3x3>& diag_inv,
    const muda::DeviceBuffer<int>&       unpart_flags,
