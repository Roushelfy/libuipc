...
class GIPCAdaptiveParameterStrategy : public AdaptiveContactParameterReporter
{
  public:
    ...
    muda::DeviceDenseVector<ActivePolicy::StoreScalar> contact_gradient;
    muda::DeviceDenseVector<ActivePolicy::StoreScalar> non_contact_gradient;
    ...
    void compute_gradient(AdaptiveParameterInfo& info)
    {
        ...
        auto _compute_gradient =
            [this](EnergyComponentFlags flags,
                   muda::DenseVectorView<ActivePolicy::StoreScalar> gradient)
        {
    ...
    virtual void do_compute_parameters(AdaptiveParameterInfo& info) override
    {
        ...
        std::vector<ActivePolicy::StoreScalar> h_contact_gradient;
        contact_gradient.copy_to(h_contact_gradient);

        std::vector<ActivePolicy::StoreScalar> h_non_contact_gradient;
        non_contact_gradient.copy_to(h_non_contact_gradient);

        auto& ctx = linear_system->ctx();

        double contact2 =
            static_cast<double>(ctx.dot(contact_gradient.cview(), contact_gradient.cview()));
        double proj =
            static_cast<double>(ctx.dot(contact_gradient.cview(), non_contact_gradient.cview()));
