...
{
template <>
class SimSystemCreator<cuda_mixed::GlobalContactManager>
{
  public:
    static U<cuda_mixed::GlobalContactManager> create(cuda_mixed::SimEngine& engine)
    {
        ...
        if(contact_enable)
            return make_unique<cuda_mixed::GlobalContactManager>(engine);
        return nullptr;
    }
};
}  // namespace uipc::backend
