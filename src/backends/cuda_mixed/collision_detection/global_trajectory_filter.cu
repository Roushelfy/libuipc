...
{
template <>
class SimSystemCreator<cuda_mixed::GlobalTrajectoryFilter>
{
  public:
    static U<cuda_mixed::GlobalTrajectoryFilter> create(cuda_mixed::SimEngine& engine)
    {
        auto contact_enable = engine.world().scene().config().find<IndexT>("contact/enable");
        if(contact_enable->view()[0])
            return make_unique<cuda_mixed::GlobalTrajectoryFilter>(engine);
        return nullptr;
    }
};
}  // namespace uipc::backend
