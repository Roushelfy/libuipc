...
{
template <>
class SimSystemCreator<cuda_al::GlobalContactManager>
{
  public:
    static U<cuda_al::GlobalContactManager> create(cuda_al::SimEngine& engine)
    {
        ...
        if(contact_enable || has_inter_primitive_constitution)
            return make_unique<cuda_al::GlobalContactManager>(engine);
        return nullptr;
    }
};
}  // namespace uipc::backend
