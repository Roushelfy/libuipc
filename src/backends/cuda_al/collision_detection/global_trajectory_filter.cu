...
{
template <>
class SimSystemCreator<cuda_al::GlobalTrajectoryFilter>
{
  public:
    static U<cuda_al::GlobalTrajectoryFilter> create(cuda_al::SimEngine& engine)
    {
        bool contact_enable = engine.world().scene().info()["contact"]["enable"];
        if(contact_enable)
            return make_unique<cuda_al::GlobalTrajectoryFilter>(engine);
        return nullptr;
    }
};
}  // namespace uipc::backend
