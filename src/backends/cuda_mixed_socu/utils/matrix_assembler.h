...
class DenseVectorAssembler
{
  public:
    ...
    MUDA_DEVICE void atomic_add(const Vector<IndexT, N>& indices,
                                ...
                                const Vector<U, M>&      G3N)
        requires(N >= 2 && M % N == 0)
    {
        constexpr int SegmentDim = M / N;
        using SegmentVector      = Vector<U, SegmentDim>;
#pragma unroll
        for(int i = 0; i < N; ++i)
        {
            int dst = indices(i);
            ...
            auto          G_store = downcast_gradient<T>(G);

            m_dense
                .template segment<SegmentDim>(dst * SegmentDim)  //
                .template atomic_add<SegmentDim>(G_store);
        }
    }
    ...
    class ProxyRange
    {
      public:
        ...
        MUDA_GENERIC void write(const Eigen::Vector<IndexT, N>& indices,
                                const Eigen::Matrix<U, N * SegmentDim, 1>& value)
            requires(N > 1)
        {
            IndexT offset = m_I;
            for(IndexT ii = 0; ii < N; ++ii)
            {
                auto G_alu = value.template segment<SegmentDim>(ii * SegmentDim);
                ElementVector G = downcast_gradient<T>(G_alu);
        ...
        template <typename U>
        MUDA_GENERIC void write(IndexT indices, const Eigen::Matrix<U, SegmentDim, 1>& value)
            requires(N == 1)
        {
            ElementVector G = downcast_gradient<T>(value);
            m_assembler.m_doublet(m_I).write(indices, G);
        }

        template <typename U>
        MUDA_GENERIC void write(IndexT indices,
                                IndexT ignore,
                                const Eigen::Matrix<U, SegmentDim, 1>& value)
            requires(N == 1)
        {
            ElementVector G = downcast_gradient<T>(value);
            if(ignore)
    ...
    class ProxyRange
    {
      public:
        ...
        MUDA_GENERIC void write(const Eigen::Vector<IndexT, N>& l_indices,
                                ...
                                const Eigen::Matrix<U, N * BlockDim, N * BlockDim>& value)
            requires(N > 1)
        {
            IndexT offset = m_I;
            for(IndexT ii = 0; ii < N; ++ii)
            {
                for(IndexT jj = 0; jj < N; ++jj)
                {
                    ...
                    ElementMatrix H = downcast_hessian<T>(H_alu);
            ...
            requires(N > 1)
        {
            ...
            for(IndexT ii = 0; ii < N; ++ii)
            {
                for(IndexT jj = 0; jj < N; ++jj)
                {
                    ...
                    {
                        auto H_alu = value.template block<BlockDim, BlockDim>(ii * BlockDim,
                                                                               jj * BlockDim);
                        H = downcast_hessian<T>(H_alu);
                    }
        ...
        template <typename U>
        MUDA_GENERIC void write(IndexT indices, const Eigen::Matrix<U, BlockDim, BlockDim>& value)
            requires(N == 1)
        {
            IndexT offset = m_I;
            ElementMatrix H = downcast_hessian<T>(value);
        ...
        MUDA_GENERIC void write(IndexT indices,
                                IndexT ignore,
                                const Eigen::Matrix<U, BlockDim, BlockDim>& value)
            requires(N == 1)
        {
            IndexT        offset = m_I;
            ElementMatrix H      = downcast_hessian<T>(value);
