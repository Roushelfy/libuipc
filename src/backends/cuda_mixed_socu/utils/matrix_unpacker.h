    ...
    class ProxyRange
    {
      public:
        ...
        MUDA_GENERIC void write(IndexT i,
                                IndexT j,
                                const Eigen::Matrix<U, BlockDim * M, BlockDim * N>& value)
            requires(BlockDim > 1 && (M > 1 || N > 1))
        {
            auto value_store = downcast_hessian<T>(value);
            IndexT offset = m_I;
            for(IndexT ii = 0; ii < M; ++ii)
            {
                for(IndexT jj = 0; jj < N; ++jj)
                {
                    m_unpacker.m_triplet(offset).write(
                        i + ii,
                        j + jj,
                        value_store.template block<BlockDim, BlockDim>(ii * BlockDim, jj * BlockDim));
                    ++offset;
                }
            }
        }
        ...
        template <typename U>
        MUDA_GENERIC void write(IndexT i, IndexT j, const Eigen::Matrix<U, M, N>& value)
            requires(BlockDim == 1 && (M > 1 || N > 1))
        {
            auto value_store = downcast_hessian<T>(value);
            IndexT offset = m_I;
            for(IndexT ii = 0; ii < M; ++ii)
            {
                for(IndexT jj = 0; jj < N; ++jj)
                {
                    m_unpacker.m_triplet(offset).write(i + ii, j + jj, value_store(ii, jj));
                    ++offset;
                }
            }
        }
        ...
        template <typename U>
        MUDA_GENERIC void write(IndexT i, IndexT j, const Eigen::Matrix<U, BlockDim, BlockDim>& value)
            requires(BlockDim > 1 && M == 1 && N == 1)
        {
            m_unpacker.m_triplet(m_I).write(i, j, downcast_hessian<T>(value));
        }


        template <typename U>
        MUDA_GENERIC void write(IndexT i, IndexT j, const U& value)
            requires(BlockDim == 1 && M == 1 && N == 1)
        {
            m_unpacker.m_triplet(m_I).write(i, j, safe_cast<T>(value));
        }
            ...
            for(IndexT ii = 0; ii < N; ++ii)
            {
                for(IndexT jj = ii; jj < N; ++jj)
                {
                    ...
                    m_unpacker.m_triplet(offset++).write(
                        dst_i,
                        dst_j,
                        value_store.template block<BlockDim, BlockDim>(ii_ * BlockDim, jj_ * BlockDim));
                }
            }
        }
