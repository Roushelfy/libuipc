...
namespace
{
struct VertexHalfPlaneNormalContactExactCache
{
...
VertexHalfPlaneNormalContactExactCache& exact_cache(std::shared_ptr<void>& cache)
{
    if(!cache)
        cache = std::make_shared<VertexHalfPlaneNormalContactExactCache>();
    return *std::static_pointer_cast<VertexHalfPlaneNormalContactExactCache>(cache);
}
...
bool VertexHalfPlaneNormalContact::do_supports_structured_hessian() const
{
    return true;
}

void VertexHalfPlaneNormalContact::do_assemble_structured_hessian(
    GlobalDyTopoEffectManager::StructuredHessianInfo& info)
{
    ...
    VertexHalfPlaneNormalContactNativeContext native_context;
...
muda::CDoubletVectorView<VertexHalfPlaneNormalContact::StoreScalar, 3> VertexHalfPlaneNormalContact::gradients() const noexcept
{
    return m_impl.gradients;
}

muda::CTripletMatrixView<VertexHalfPlaneNormalContact::StoreScalar, 3> VertexHalfPlaneNormalContact::hessians() const noexcept
{
...
namespace uipc::backend::cuda_mixed
{
using StoreVec3 = Eigen::Matrix<VertexHalfPlaneNormalContact::StoreScalar, 3, 1>;
using StoreMat3 = Eigen::Matrix<VertexHalfPlaneNormalContact::StoreScalar, 3, 3>;
