        ...
        ParallelFor()
            ...
            .apply(
                     ...
                     ExternalArticulationConstituion::RevoluteJointConstitutionUID] __device__(IndexT I) mutable
                {
                    ...
                    Alu     m_ij = safe_cast<Alu>(joint_joint_id_to_mass(I));
                    Vector2i ij   = joint_joint_id_to_joint_ij(I);

                    auto compute_delta_theta = [&](IndexT joint_id) -> Alu
                    {
                        Eigen::Matrix<Alu, 6, 1> basis_k =
                            joint_id_to_L_basis(joint_id).template cast<Alu>();
                        Eigen::Matrix<Alu, 6, 1> basis_l =
                            joint_id_to_R_basis(joint_id).template cast<Alu>();
                        Vector2i body_ids = joint_id_to_body_ids(joint_id);

                        Eigen::Matrix<Alu, 12, 1> qk =
                            qs(body_ids[0]).template cast<Alu>();
                        Eigen::Matrix<Alu, 12, 1> ql =
                            qs(body_ids[1]).template cast<Alu>();

                        const Vector2i& ref_q_prev_ids =
                            joint_id_to_ref_q_prev_ids(joint_id);

                        Eigen::Matrix<Alu, 12, 1> q_prevk =
                            (ref_q_prev_ids[0] >= 0 ? ref_q_prevs(ref_q_prev_ids[0]) :
                                                      q_prevs(body_ids[0]))
                                .template cast<Alu>();

                        Eigen::Matrix<Alu, 12, 1> q_prevl =
                            (ref_q_prev_ids[1] >= 0 ? ref_q_prevs(ref_q_prev_ids[1]) :
                                                      q_prevs(body_ids[1]))
                                .template cast<Alu>();
                        ...
                        {
                            ERJ::DeltaTheta<Alu>(
                                delta_theta, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                        }
                    ...
                    Alu delta_theta_i = compute_delta_theta(ij[0]);
                    Alu delta_theta_j = compute_delta_theta(ij[1]);

                    Alu delta_theta_tilde_i =
                        safe_cast<Alu>(joint_id_to_delta_theta_tilde(ij[0]));
                    Alu delta_theta_tilde_j =
                        safe_cast<Alu>(joint_id_to_delta_theta_tilde(ij[1]));
        ...
        FastSegmentalReduce()
            ...
            .reduce(
                     ...
                     ExternalArticulationConstituion::PrismaticJointConstitutionUID] __device__(IndexT joint_jointI) -> InternalScalar
                {
                    ...
                    Alu     m_ij = safe_cast<Alu>(joint_joint_id_to_mass(joint_jointI));
                    ...
                    {
                        // should not reach here
                        delta_theta_j = safe_cast<Alu>(0.0);
                    }
        ...
        ParallelFor()
            ...
            .apply(
                     ...
                     ExternalArticulationConstituion::PrismaticJointConstitutionUID] __device__(IndexT jointI) mutable
                {
                    ...
                    Eigen::Matrix<Alu, 6, 1> basis_k =
                        joint_id_to_L_basis(jointI).template cast<Alu>();
                    Eigen::Matrix<Alu, 6, 1> basis_l =
                        joint_id_to_R_basis(jointI).template cast<Alu>();
                    ...
                    if(joint_uid == RevoluteUID)
                    {
                        ERJ::dDeltaTheta_dQ<Alu>(
                            G24, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                    }
                    else if(joint_uid == PrismaticUID)
                    {
                        EPJ::dDeltaTheta_dQ<Alu>(
                            G24, basis_k, qk, q_prevk, basis_l, ql, q_prevl);
                    }
                    else
                    {
        ...
        ParallelFor()
            ...
            .apply(
                     ...
                     ExternalArticulationConstituion::PrismaticJointConstitutionUID] __device__(IndexT joint_joint_I) mutable
                {
                    ...
                    auto compute_dDeltaTheta_dQ = [&](IndexT joint_id) -> Eigen::Matrix<Alu, 24, 1>
                    {
                        U64 joint_uid = joint_id_to_uid(joint_id);
                        ...
                        Eigen::Matrix<Alu, 24, 1> dDeltaTheta_dQ;
                    ...
                    Eigen::Matrix<Alu, 24, 1> dDeltaTheta_dQ_i =
                        compute_dDeltaTheta_dQ(ij[0]);
                    Eigen::Matrix<Alu, 24, 1> dDeltaTheta_dQ_j =
                        compute_dDeltaTheta_dQ(ij[1]);


                    Eigen::Matrix<Alu, 24, 24> H24x24 =
                    ...
                    if(ij[0] == ij[1])
                    {
                        ...
                        Eigen::Matrix<Alu, 6, 1> basis_k =
                            joint_id_to_L_basis(i).template cast<Alu>();
                        Eigen::Matrix<Alu, 6, 1> basis_l =
                            joint_id_to_R_basis(i).template cast<Alu>();
                        ...
                        Alu G_theta_i = safe_cast<Alu>(joint_id_to_G_theta(i));

                        Eigen::Matrix<Alu, 12, 1>  F, F_t;
                        Eigen::Matrix<Alu, 24, 24> JT_H_J;
                        if(joint_uid == RevoluteUID)
                        {
                            ERJ::F<Alu>(F, basis_k, qk, basis_l, ql);
                            ERJ::F<Alu>(F_t, basis_k, q_prevk, basis_l, q_prevl);
                            Eigen::Matrix<Alu, 12, 12> ddDeltaTheta_ddF;
                            ERJ::ddDeltaTheta_ddF(ddDeltaTheta_ddF, F, F_t);
                            Eigen::Matrix<Alu, 12, 12> HF = G_theta_i * ddDeltaTheta_ddF;
                            make_spd(HF);
                            ERJ::JT_H_J<Alu>(
                                JT_H_J, HF, basis_k, basis_l, basis_k, basis_l);
                        }
                        else if(joint_uid == PrismaticUID)
                        {
                            EPJ::F<Alu>(F, basis_k, qk, basis_l, ql);
                            EPJ::F<Alu>(F_t, basis_k, q_prevk, basis_l, q_prevl);
                            Eigen::Matrix<Alu, 12, 12> ddDeltaTheta_ddF;
    ...
    void do_update_state(UpdateVelocityInfo& info) override
    {
        ...
        ParallelFor()
            ...
            .apply(
                ...
                {
                    using Alu = ActivePolicy::AluScalar;
                    Eigen::Matrix<Alu, 6, 1> L_basis =
                        joint_id_to_L_basis(jointI).template cast<Alu>();
                    Eigen::Matrix<Alu, 6, 1> R_basis =
                        joint_id_to_R_basis(jointI).template cast<Alu>();
                    Vector2i body_ids = joint_id_to_body_ids(jointI);
