...
def configure_solver(config: Any, variant: str, workspace: Path) -> None:
    ...
    socu["native_contact_plan"] = (
        1 if os.environ.get("SOCU_NATIVE_CONTACT_PLAN") == "1" else 0
    )
    socu["native_contact_plan_executor"] = (
        1 if os.environ.get("SOCU_NATIVE_CONTACT_PLAN_EXECUTOR") == "1" else 0
    )
    native_contact_side_coverage_mode = os.environ.get(
        "SOCU_NATIVE_CONTACT_SIDE_COVERAGE_MODE"
    )
