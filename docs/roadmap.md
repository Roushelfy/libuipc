...
- [ ] Step 2: per-primitive beta. Evolve beta for every VT primitive keyed by the orientation-invariant VT key, using the **true closest-feature distance** regardless of which sub-formula is active; carry beta across PP/PE/PT transitions; stop zero-filling `m_beta_PE/PP`.
...
- [ ] Step 5: bonded lock decides on the VT primitive (beta now exists for all of them) so corner/edge contacts bond; the ABD tet stays point-plane. Re-check `pt_lift_release` stays penetration-free.
...
| Adhesion and bonding are PT-only (Phase 6) | Stable edge/corner contacts get no adhesion and cannot bond; the off-diagonal cube corners are excluded by construction | Land per-primitive beta (Phase 6 Step 2) on top of the Step-1 PE/PP formulas, then let the bonded lock decide on the VT primitive (Step 5) |
