...
4. Lock is allowed only after beta (or, in distance-lock mode, the distance band), age, sticky-side, normal-gap, tangential-slip, and rest-shape conditioning gates pass.
5. Release must carry a beta value back to RCC persistence; release cannot silently reset beta or delete pair history. (Beta mode; in distance-lock mode beta does not exist and the carry is vacuous — see Phase 7.)
...
- [x] Step 5: bonded lock decides on the VT primitive, restricted to geometrically-sound (face-interior) bonds. The producer is fed the full `friction_VTs` topologies (`m_vt_topos`, aligned 1:1 with `m_beta_PT`), but with a lock-beta (`m_vt_lock_beta`) masked to 0 on non-face-interior (closest-feature dim != 4) VTs. Rationale: the bonded ABD tet is a point-PLANE bond, sound only when the point projects INSIDE the triangle; for an edge/corner VT the point projects outside the triangle footprint and a point-plane tet is a skewed sliver. So edge/corner VTs keep full-feature adhesion (Step 2/3) but do NOT bond. The filter's locked-key compact also removes locked (face) VTs from `friction_VTs` (new `rcc_bonded_pt_unlocked_VT`), so a bonded pair is never both adhered and bonded (no double-count); the ABD tet stays point-plane (`F = Ds Dm_inv`). Verified: `[rcc_bonded_pt][scene][pt_lift_release]` 596/1, backend `[rcc_bonded_pt]` 247/14, oracle 12/4, bunny 4/1, legacy `[rcc_adhesion][gate]` 836/2 all green. Headless probe (faceted subdivided cube): 8 sound face-interior bonds (all projection-inside), 88 edge/corner VTs adhere at beta=1.0 without bonding, cube carried through the lift, no NaN, no spurious tets. (The first attempt fed unmasked beta and bonded all 96 VTs, producing 88 skewed point-outside-triangle sliver tets — fixed by the face-interior mask; see journal 2026-06-03.)
...
## Phase 7: Distance-Locked Bonding Without Adhesion Energy (Implemented)

An alternative bonded mode for scenes that want the virtual-tet bond/release machinery with **no soft adhesion force**: adhesion energy is not assembled, beta does not exist, and the lock gate is `d < ξ + c·d_hat` on the end-of-step true closest-feature distance, with user coefficient `c ∈ [0,1]` (`rcc_bonded_pt_distance_lock_ratio`; `ξ = 0` reduces to `d < c·d_hat`). Release gates are unchanged — they are already beta-free. Feasibility was verified against source on 2026-06-10 and the implementation landed 2026-06-11 (journal "Distance-Locked Bonding Feasibility" / "Phase 7 Implementation"); see [architecture](./architecture.md) "Distance-Locked Bonding Without Adhesion Energy" for the design of record, including why `adhesion_enabled = 0` and `Cn = 0` cannot express this mode (beta pinning / policy release / 0-division) and why the lock driver stays Phase A of the adhesive reporter.

- [x] Step 1: config keys `rcc_bonded_pt_distance_lock` (IndexT, default 0) and `rcc_bonded_pt_distance_lock_ratio` (Float, default 0.5, clamped to `[0,1]` on read), read in `RCCBetaEvolutionTimeIntegrator::do_build`, plus the contradictory-config warning (mode on + adhesion-enabled row with `Cn`/`Ct > 0` → coefficients ignored, reported once at tabular build).
- [x] Step 2: distance-mode lock-eligibility kernel — recompute the closest-feature flag from end-of-step positions, flagged `D`, predicate `D < (ξ + c·d_hat)²` (shared `VT_distance_lock_band_pass`, structurally false at `c <= 0`), composed with `adhesion_enabled`, the fused occlusion cast (shared `VT_occlusion_blocked`, also rewired into the Phase B kernels), sticky-side parity, and the face-interior mask (shared `VT_lock_face_interior_pass`); emit indicator lock-beta 1.0/0.0 so the bonded producer and release path run unmodified. The global lock threshold is clamped to `<= 1` in BOTH paths that can veto an indicator lock: the producer-call scalar AND the per-pair `bonded_lock_threshold` sentinel resolution (an unclamped sentinel fallback silently vetoed every lock in the first implementation round). CPU oracle `[rcc_bonded_pt][oracle][distance_lock]` (18 assertions) checks the band predicate against an exact brute-force closest-distance reference plus the face-foot gate.
- [x] Step 3: energy/beta bypass — `friction_pair_counts` reports 0 AND the energy/assemble kernel bodies are gated together; Phase B beta init, Phase A beta evolution, and the released-beta carry are skipped together; the vertex-normal recompute and release-context wiring keep running; the occlusion cast moved into the eligibility kernel (end-of-step positions — see the design note).
- [x] Step 4: scene gate `[rcc_bonded_pt][scene][distance_lock]` (604 assertions green): cube press/hold/lift with `Cn = Ct = 0`, `rcc_bonded_pt_lock_face_interior_only = 1`, `rcc_bonded_pt_distance_lock_ratio = 0.95` (the fixture's press equilibrium gap is ~0.0178 ≈ 0.89·d_hat — a band must cover the gap the press actually reaches), a deliberately mis-set beta-mode global threshold 1.5 (proves the clamp), and `release_gap = 0.05 > c·d_hat` (anti-churn guidance): bonds form by distance alone, no penetration with CCD skipped, the cube is carried purely by the bonded energy, the accessor dump carries sentinel beta 1.0, the forced pull releases, and full separation ends at zero locks.
- [x] Step 5: `RCCBondedPTCounters.distance_rejected_count` (distance band + occlusion rejections) and `.policy_rejected_count` (explicit `adhesion_enabled` rejections) — report names `rcc_bonded_pt_rejected_distance_count` / `rcc_bonded_pt_rejected_policy_count` — wired from the eligibility kernel via `RCCBondedPTSystem::add_lock_rejection_counts`, exposed through the pybind counters dict, covered by the `[rcc_bonded_pt][state][counters]` contract fixture, and doc status flips.
...
| ~~Bonding is PT-only~~ (resolved, Phase 6 Step 5) | Resolved: the producer locks on the VT primitive but restricted to face-interior (projection-inside) pairs, because the point-plane ABD tet is only sound there; edge/corner VTs adhere (Step 2/3) but do not bond. Locked VTs leave `friction_VTs` (no double-count). `pt_lift_release` penetration-free (596/1); faceted cube forms 8 sound bonds + 88 adhering VTs. ABD tet stays point-plane | — |
...
| Distance-lock band oracle | `build/cuda_mixed_fused_pcg/RelWithDebInfo/bin/uipc_test_backend_cuda "[rcc_bonded_pt][oracle][distance_lock]" -r compact` | `VT_distance_lock_band_pass` matches an exact brute-force closest-distance reference (PT/PE/PP features, xi > 0, c = 0 and exact-boundary cases) and the face-foot gate behaves per spec |
| Distance-lock scene | `build/cuda_mixed_fused_pcg/RelWithDebInfo/bin/uipc_test_sim_case "[rcc_bonded_pt][scene][distance_lock]" -r compact` | With `rcc_bonded_pt_distance_lock` on and `Cn = Ct = 0`: bonds form by `d < ξ + c·d_hat` during press/hold, the cube is carried purely by the bonded energy, sentinel beta 1.0 round-trips the accessor dump, the distance-rejection counter is live, the forced pull releases via the unchanged gates, and full separation ends at zero locks — penetration-free with CCD skipped |
