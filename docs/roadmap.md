...
Current focus: **the no-penetration scene gate**, which unblocks safely enabling the now-implemented pre-CCD skip. The bonded reporter (Phase 3), the release/fallback machinery (Phase 4), and the pre-CCD filter (Phase 2) are implemented for the current scope. There are three independent performance levers, not one: (1) locked pairs are removed from barrier/friction/RCC assembly — active now; (2) replacing their stiff near-contact log-barrier Hessian with a smooth high-kappa ABD block can improve linear-system conditioning and cut Newton/PCG iteration counts — active now and likely the dominant win; (3) PT CCD broadphase/TOI cost — the skip is now implemented in all four simplex filters (locked pairs are rejected inside the PT broadphase predicate via the shared `rcc_bonded_pt_candidate_is_locked` helper), but gated behind default-off `rcc_bonded_pt_skip_ccd`, so there are still **zero CCD savings** by default. Removing CCD also removes the last non-penetration guard, so `rcc_bonded_pt_skip_ccd` stays off until the no-penetration scene gate passes (see the architecture CCD Removal Precondition). No performance claim until measured (Newton/PCG iteration counts plus per-stage timing, swept over `rcc_bonded_pt_kappa`). The live lock gate is also still beta/rest-shape only, released reason snapshots are not yet scene-accessible, and lifecycle scene correctness is unproven.
...
| Pre-CCD skip is implemented but default-off | The skip mechanism now rejects locked PTs in every simplex filter's PT broadphase predicate, but `rcc_bonded_pt_skip_ccd` defaults off because removing CCD removes the last non-penetration guard. So there are no CCD savings by default | Add the no-penetration scene gate so `rcc_bonded_pt_skip_ccd` can be enabled safely |
...
Add the `pt_lift_release` no-penetration scene observation: it is the gate that lets `rcc_bonded_pt_skip_ccd` (the now-implemented pre-CCD skip) be enabled safely, since the bonded ABD energy is reflection-invariant and cannot by itself prevent tunneling. Then expose release-reason scene diagnostics, add lock-gate parity, and benchmark (Newton/PCG iteration counts + per-stage timing, swept over `rcc_bonded_pt_kappa`). Keep bonded mode and `rcc_bonded_pt_skip_ccd` default-off and make no correctness/performance claims until the no-penetration gate, lock gates, scene diagnostics, and benchmark gates all pass.
