...
| Bonding is still PT-only (Phase 6 Step 5) | Adhesion now covers all VT primitives (per-VT beta + flagged feature distance, Step 2/3 done), so edge/corner contacts get adhesion. But the bonded producer is still fed only the `flag==4` (face-interior) subset, so corner/edge contacts get adhesion yet cannot bond | Let the bonded producer consume the full `friction_VTs` list + per-VT beta (Step 5); re-check `pt_lift_release` stays penetration-free |
