...
| Builder-generated ABD lanes match legacy projection | Planned builder lane fixture checks component/weight for nontrivial `x_bar` | Source scan keeps ABD projection blocker in roadmap | Direct/compare scenes cannot replace this unit gate |
| Scalar diag compatibility changes builder output | Planned builder fixture checks `DiagScalarFem` and `DiagScalarAbd` task emission | Source scan keeps scalar diag blocker in roadmap | Report gate checks scalar diag counts when the mode is benchmarked |
