...
- [x] Add a source-order mutation contract proving every consumer observes the
  same `source_id`.
...
| Source-order mutation contract | `build/socu_native_contact/RelWithDebInfo/bin/uipc_test_backend_cuda_mixed_socu "cuda_mixed_socu_contact_executor_source_id_indexed_triplets"` and `build/socu_native_contact/RelWithDebInfo/bin/uipc_test_backend_cuda_mixed_socu "cuda_mixed_socu_contact_direct_evaluator_per_family_triplet_parity"` | Shuffled builder source inputs still produce source-id indexed program, triplet, and direct-evaluator behavior |
