        ...
        ParallelFor()
            ...
            .apply(info.energies().size(),
                    ...
                    Pi] __device__(int I)
                   {
                       ...
                       const Alu k = safe_cast<Alu>(bending_stiffnesses(I) * dt * dt);
                       const Alu r = safe_cast<Alu>(thicknesses(I));
                       const Alu pi = safe_cast<Alu>(Pi);

                       Eigen::Matrix<Alu, 9, 1> X;
                       X.segment<3>(0) = xs(hinge[0]).template cast<Alu>();
                       X.segment<3>(3) = xs(hinge[1]).template cast<Alu>();
                       X.segment<3>(6) = xs(hinge[2]).template cast<Alu>();

                       auto x0_bar = x_bars(hinge[0]).template cast<Alu>();
                       auto x1_bar = x_bars(hinge[1]).template cast<Alu>();
                       auto x2_bar = x_bars(hinge[2]).template cast<Alu>();

                       // Rest length of the two edges
                       const Alu L0 = (x1_bar - x0_bar).norm() + (x2_bar - x1_bar).norm();
        ...
        ParallelFor()
            ...
            .apply(hinges.size(),
                    ...
                    gradient_only = info.gradient_only()] __device__(int I) mutable
                   {
                       ...
                       const Alu dt2 = safe_cast<Alu>(dt * dt);

                       Eigen::Matrix<Alu, 9, 1> G_alu;
