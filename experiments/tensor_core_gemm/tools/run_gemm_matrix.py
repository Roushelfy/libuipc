...
from gemm_registry import ALL_SHAPES, benchmark_name, smoke_batches
...
def find_test_exe(build_dir: Path, config: str) -> Path:
    exe_name = "tensor_core_gemm_test.exe" if os.name == "nt" else "tensor_core_gemm_test"
    candidates = [
        build_dir / config / "bin" / exe_name,
        build_dir / "bin" / exe_name,
    ]
...
def smoke_filter() -> str:
    names: list[str] = []
    for shape in ALL_SHAPES:
        for layout in ("Raw", "Padded"):
            for batch in smoke_batches(shape):
                for mode in ("Fp64", "Fp32", "Tc32"):
                    names.append(benchmark_name(layout, mode, shape, batch))
    return "^(" + "|".join(re.escape(name) for name in names) + ")$"


def main() -> None:
    parser = argparse.ArgumentParser(description="Run tensor_core_gemm benchmark matrix.")
    ...
    parser.add_argument("--benchmark_min_time", default="0.01s")
    ...
    root = (
        ...
        if args.run_root
        else Path("output") / "tensor_core_gemm" / datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    raw_dir = root / "raw"
    tables_dir = root / "tables"
    per_shape_dir = root / "per_shape"
    ...
    benchmark_filter = args.benchmark_filter or (smoke_filter() if args.profile == "smoke" else ".*")
    ...
    meta = {
        ...
        "ncu_profile": str(args.ncu_profile.resolve()) if args.ncu_profile else None,
    ...
    if args.ncu_profile:
        aggregate_cmd.extend(["--ncu_profile", str(args.ncu_profile.resolve())])
