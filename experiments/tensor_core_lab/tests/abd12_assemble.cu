...
TEST_CASE("tensor_core_lab_abd12_assemble", "[tensor_core_lab][abd12]")
{
    using namespace uipc::tensor_core_lab;

    const auto data = make_abd12_assemble_case("unit_abd12_assemble", 32, 43, 1.0e4);

    const auto fp64 = run_abd12_assemble_case(Mode::Fp64RefNoTc, data);
    ...
    CHECK(fp64.metrics.rel_fro < 5.0e-8);
    CHECK(fp64.metrics.nan_inf_count == 0);
    CHECK(fp64.metrics.symmetry_error < 1.0e-10);

    const auto fp32 = run_abd12_assemble_case(Mode::Fp32NoTc, data);
    ...
    CHECK(fp32.metrics.rel_fro < 1.0e-4);
    CHECK(fp32.metrics.nan_inf_count == 0);
    CHECK(fp32.metrics.symmetry_error < 5.0e-4);

    const auto tc32 = run_abd12_assemble_case(Mode::Tc32Tf32, data);
