...
TEST_CASE("tensor_core_lab_mas48_solve", "[tensor_core_lab][mas48]")
{
    using namespace uipc::tensor_core_lab;

    const auto data = make_mas48_case("unit_mas48_solve", 256, 41, 1.0e2);

    const auto fp64 = run_spd_case(Mode::Fp64RefNoTc, OpKind::Mas48Solve, data);
    ...
    const auto fp32 = run_spd_case(Mode::Fp32NoTc, OpKind::Mas48Solve, data);
    ...
    const auto tc32 = run_spd_case(Mode::Tc32Tf32, OpKind::Mas48Solve, data);
