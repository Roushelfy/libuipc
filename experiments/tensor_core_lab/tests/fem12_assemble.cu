...
TEST_CASE("tensor_core_lab_fem12_assemble", "[tensor_core_lab][fem12]")
{
    using namespace uipc::tensor_core_lab;

    const auto data = make_fem12_case("unit_fem12", 32, 7, 1.0e4);

    const auto fp64 = run_fem12_case(Mode::Fp64RefNoTc, data);
    ...
    const auto fp32 = run_fem12_case(Mode::Fp32NoTc, data);
    ...
    const auto tc32 = run_fem12_case(Mode::Tc32Tf32, data);
