...
TEST_CASE("tensor_core_lab_abd12_solve", "[tensor_core_lab][abd12]")
{
    using namespace uipc::tensor_core_lab;

    const auto data = make_abd12_case("unit_abd12_solve", 4096, 43, 1.0e2);

    const auto fp64 = run_spd_case(Mode::Fp64RefNoTc, OpKind::Abd12Solve, data);
    ...
    const auto fp32 = run_spd_case(Mode::Fp32NoTc, OpKind::Abd12Solve, data);
    ...
    const auto tc32 = run_spd_case(Mode::Tc32Tf32, OpKind::Abd12Solve, data);
