...
void bench_joint24(benchmark::State& state, Mode mode)
{
    ...
    if(mode == Mode::Fp64RefNoTc)
    {
        ...
        run_bench(state, data, prepared, context, execute_joint24_case);
    }
    else
    {
...
void BM_Joint24Fp64(benchmark::State& state) { bench_joint24(state, Mode::Fp64RefNoTc); }
void BM_Joint24Fp32(benchmark::State& state) { bench_joint24(state, Mode::Fp32NoTc); }
void BM_Joint24Tc32(benchmark::State& state) { bench_joint24(state, Mode::Tc32Tf32); }
}  // namespace

BENCHMARK(BM_Joint24Fp64)->Args({2048, 2})->Args({16384, 4})->UseManualTime();
BENCHMARK(BM_Joint24Fp32)->Args({2048, 2})->Args({16384, 4})->UseManualTime();
BENCHMARK(BM_Joint24Tc32)->Args({2048, 2})->Args({16384, 4})->UseManualTime();
