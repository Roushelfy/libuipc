...
{
    if(data.aux.size() == 0)
        return {RunStatus::InvalidArgument, 0.0, {}, "abd12_assemble expects aux term"};

    RunOutcome out;

    const int dim = data.spec.shape.physical_rows;
    if(dim != data.spec.shape.physical_cols)
        return {RunStatus::InvalidArgument, 0.0, {}, "abd12_assemble expects square padded matrices"};
    ...
    add_inplace<<<grid_size, block_size>>>(data.output.data(), data.aux.data(), count);
...
__global__ void abd12_wmma_kernel(const float* left,
                                  const float* middle,
                                  const float* aux,
                                  float*       output,
                                  int          batch_count)
{
    ...
    __shared__ float s_out[matrix_stride];
    ...
    const float* aux_batch    = aux + static_cast<size_t>(batch) * matrix_stride;
    ...
    wmma::store_matrix_sync(s_out, acc1, dim, wmma::mem_col_major);
    __syncthreads();

    for(int i = tid; i < matrix_stride; i += blockDim.x)
        output[static_cast<size_t>(batch) * matrix_stride + i] = s_out[i] + aux_batch[i];
#else
    (void)left;
    (void)middle;
    (void)aux;
                                ...
                                bool             measure_time)
{
    ...
    if(dim != data.spec.shape.physical_cols || dim != 16)
        return {RunStatus::InvalidArgument,
                0.0,
                {},
                "abd12_assemble tc_wmma expects 16x16 padded matrices"};
    ...
    abd12_wmma_kernel<<<data.spec.batch_count, 32>>>(
        data.left.data(),
        data.middle.data(),
        data.aux.data(),
        data.output.data(),
...
RunOutcome execute_abd12_assemble_case(BackendContext& context,
                                       PreparedCaseF32& data,
                                       bool             measure_time)
{
